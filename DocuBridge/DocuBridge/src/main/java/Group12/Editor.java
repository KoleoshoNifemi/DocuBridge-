    ...
    private void fontSizeShortcut(String operand, String source){
        Platform.runLater(() -> {
            quill.executeScript("quill.focus();" +
                            ...
                            "    currentSize = '13px';" +
                            "}" +
                            "var number = parseInt(currentSize);" +
                            "var newSize = (number " + operand + " 2) + 'px';" +
    ...
    private void applyScript(String scriptType, String source){
        Platform.runLater(() -> {
           quill.executeScript("quill.focus();" +
                   "quill.history.cutoff();" +
                   "quill.history.cutoff();" +
                   "var currentScript = quill.getFormat().script;" +    // Get the current script format ('sub', 'super', or undefined)
                   // If it matches the button we clicked, turn it off (false). Otherwise, turn it on.
